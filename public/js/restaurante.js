...
const formOpinar = document.getElementById("frmopinar");
...
document.getElementById("frmopinar").addEventListener("submit", function (event) {
    event.preventDefault();
    let estrellas = document.querySelector('input[name="estrellas"]:checked');
    ...
    if (!estrellas) {
        errorEstrellas.textContent = "Debe seleccionar una calificación.";
        valido = false;
    }
    if (comentario.length < 5) {
        errorComentario.textContent = "El comentario debe tener al menos 5 caracteres.";
        valido = false;
    }
